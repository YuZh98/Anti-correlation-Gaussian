### Demo: variable selection in a linear regression with the L1-ball-prior
### Gibbs sampler.
###
### Run from the directory containing this file:   Rscript LinearReg.R
### or interactively:                               source("LinearReg.R")

source("SliceSampler.R")
source("SourceCode.R")
if (!requireNamespace("ggplot2", quietly = TRUE)) {
  stop("This demo requires 'ggplot2'.")
}
library(ggplot2)
...
n      <- 300
p      <- 500
d0     <- 10    # Number of non-zero entries
rho0   <- 0.5   # Cor(x_i, x_j) = rho^|i-j|
sig2_0 <- 1     # True error variance
c0     <- 3     # SNR coefficient for |beta_j|
...
theta0      <- rep(0, p)
theta0[1:d0] <- c0 * sqrt(sig2_0 * log(p) / n) *
  c(2, -3, 2, 2, -3, 3, -2, 3, -2, 3)
...
fit <- l1ball.linreg(X, y, sig20 = 1, tau0 = rep(1, p), K0 = 1,
                     ...
                     init_method = "random")
cat("The total running time for the Blocked Gibbs sampler is ",
    fit$running.time, " seconds.\n")
...
par(mfrow = c(1, 1))
if (p < 50) {
  boxplot(fit$trace_theta, outline = FALSE)
  points(theta0, col = "blue", pch = 19)
} else {
  boxplot(fit$trace_theta[, 1:50], outline = FALSE)  # first 50 variables
...
lag.max <- 40
for (i in 1:10) {
  ACF    <- acf(fit$trace_theta[, i], lag.max = lag.max, plot = FALSE)
  df.new <- data.frame(LAGS = 0:lag.max, acf = ACF$acf[, 1, 1])
  df.ACF <- rbind(df.ACF, df.new)
}
ggplot(df.ACF, aes(x = factor(LAGS), y = acf)) +
